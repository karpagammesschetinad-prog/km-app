...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const [salesRows, expenseRows, categoryTypes] = await Promise.all([getAllRows(SHEETS.SALES), getAllRows(SHEETS.EXPENSES), getAllRows(SHEETS.EXPENSE_CATEGORY_TYPES)]);
    ...
    const totalSales = Object.values(shiftSales).reduce((total, value) => total + value, 0);
    ...
    res.json({ success: true, data: { date, rows: salesRows.map(rowToObj), summary: { totalSales, expenseTotal: dailyExpenseTotal, remaining: totalSales - dailyExpenseTotal, remainingByShift: remainingByShiftWithAliases, shiftExpenses: shiftExpensesWithAliases, shiftSales: shiftSalesWithAliases } } });
